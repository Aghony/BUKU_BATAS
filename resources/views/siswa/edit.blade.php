<x-layout>
    ...
    <div class="bg-white sm:w-451 w-full h-auto flex flex-col shadow-md mx-auto p-5 gap-4 justify-between rounded-md ">
        ...
        <div class="flex flex-col">
            <form id="inputForm" action="{{ route('siswa.update', $siswas->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="flex flex-col justify-between gap-2 ">
                    <label for="jurusan_id">
                        ...
                        <select name="jurusan_id" id="jurusan_id"
                            ...
                            @foreach ($jurusans as $jurusan)
                                <option
                                    value="{{ $jurusan->id }}"{{ $data->jurusan_id == $jurusan->id ? 'selected' : '' }}>
                                    {{ $jurusan->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    ...
                    <label for="sub_kelas">
                        ...
                        <select name="sub_kelas" id="sub_kelas"
                            ...
                            @foreach ($subKelasOptions as $subkelas)
                                <option
                                    value="{{ $subkelas }}"{{ $data->sub_kelas == $subkelas ? 'selected' : '' }}>
                                    {{ $subkelas }}</option>
                            @endforeach
                        </select>
                    </label>
                    ...
                    <label for="nama">
                        ...
                        <input
                            ...
                            type="text" placeholder="Nama..." value="{{ old('nama', $siswas->nama) }}"
                    ...
                    <label for="gender">
                        ...
                        <select name="gender" id="gender"
                            ...
                            @foreach ($genderOptions as $genders)
                                <option value="{{ $genders }}"{{ $data->gender == $genders ? 'selected' : '' }}>
                                    {{ $genders }}</option>
                            @endforeach
                        </select>
                    </label>
                    ...
                    <label for="nisn">
                        ...
                        <input
                            class="w-full h-12 bg-white border border-slate-700 rounded-md px-3 py-2 focus:outline-none"
                            type="number" placeholder="Nisn..." value="{{ old('nisn', $siswas->nisn) }}"
                            id="nisn" name="nisn">
                    </label>
                    ...
                    <label for="tanggal_lahir">
                        ...
                        <input
                            class="w-full h-12 bg-white border border-slate-700 rounded-md px-3 py-2 focus:outline-none"
                            type="date" value="{{ old('tanggal_lahir', $siswas->tanggal_lahir) }}"

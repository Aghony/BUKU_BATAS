    ...
    <div class="bg-white w-full lg:w-850 h-auto flex flex-col p-5 gap-4 shadow-md rounded-md mx-auto">
        ...
        <div class="w-full h-full flex sm:flex-row flex-col gap-5">
            ...
            {{-- <a class="bg-merah-0 w-40 h-9 rounded-lg flex flex-row items-center justify-center text-white font-bold gap-2"
                        href="{{ url('matapelajaran/export/excel') }}">
                        Download Excel
                        </a> --}}

            <a href="{{ route('matapelajaran.import') }}"
            class="bg-merah-0 hover:bg-red-700 w-auto  h-9 px-3 rounded-md flex flex-row items-center justify-center text-white font-medium gap-2">
            Upload Data Matapelajaran
        </a>
            ...
            <div class=" overflow-auto">
                <table id="myTable" class="table w-full text-left ">
                    ...
                    <tbody class="bg-white text-gray-500">
                        @foreach ($matapelajaran as $matapelajaran2)
                            <tr class="py-1">
                                <td class="py-1 border text-start font-semibold p-4">{{ $loop->iteration }}</td>
                                <td class="py-1 border text-start font-semibold p-4">{{ $matapelajaran2->name }}</td>
                                <td class="py-1 border text-start font-semibold p-4">{{ $matapelajaran2->nick }}</td>
                                <td class="py-1 border text-center font-semibold p-4"><a
                                        href="{{ route('matapelajaran.edit', $matapelajaran2->id) }}"
                                        class="border border-gray-500 hover:bg-gray-500 rounded p-2 text-gray-500 hover:text-white ">Edit</a>
                                </td>
                                <td class="py-1 border text-center items-center p-4 w-5 h-5">
                                    <form action="{{ route('matapelajaran.destroy', $matapelajaran2->id) }}"
        ...
        function hapus(event, form) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire(
                        'Terhapus!',
                        'Data Matapelajaran telah dihapus.',
                        'success'
                    );
                }
            });

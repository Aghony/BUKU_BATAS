<x-layout>
    ...
    <div class="bg-white sm:w-500 w-full h-auto flex flex-col shadow-md mx-auto p-5 gap-4 justify-between rounded-md ">
        <h1 class="text-xl ml-3 font-bold ">
            Nambah Matapelajaran 
        </h1>

        <div class="flex flex-col ">
            <form id="inputForm" action="{{ route('matapelajaran.store') }}" method="POST">
                @csrf
                ...
                <label for="name">
                    ...
                    <input class="w-full h-12 bg-white border border-slate-700 rounded-md px-3 py-2 focus:outline-none" 
                    type="text" placeholder="Matapelajaran..." value="{{ old('name') }}" id="name" name="name" required>
                </label>
                <label for="nick">
                    <span class="block font-semibold text-slate-700">
                        Singkatan 
                    ...
                    <input class="w-full h-12 bg-white border border-slate-700 rounded-md px-3 py-2 focus:outline-none" 
                    type="text" placeholder="Singkatan..." value="{{ old('nick') }}" id="nick" name="nick" required>
                </label>
            </div>

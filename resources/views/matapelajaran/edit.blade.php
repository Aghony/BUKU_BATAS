<x-layout>
    ...
    <div class="bg-white sm:w-500 w-full h-auto flex flex-col shadow-md mx-auto p-5 gap-4 justify-between rounded-md ">
        ...
        <div class="flex flex-col">
            <form id ="inputForm" action ="{{ route('matapelajaran.update', $matapelajaran->id) }}" method ="POST">
                @csrf
                @method('PUT')
                ...
                <label for="name">
                    ...
                    class="w-full h-12 bg-white border border-slate-700 rounded-md px-3 py-2 focus:outline-none"
                        type="text" placeholder="matapelajaran..." value="{{ old('name', $matapelajaran->name) }}" id="name"
                        name="name" required>
                </label>
                <label for="nick">
                    ...
                    <input
                    class="w-full h-12 bg-white border border-slate-700 rounded-md px-3 py-2 focus:outline-none"
                        type="text" placeholder="Singkatan..." value="{{ old('nick', $matapelajaran->nick) }}"
